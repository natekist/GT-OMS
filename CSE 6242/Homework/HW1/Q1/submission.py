...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        if (source, target) not in self.edges and source != target and (target, source) not in self.edges:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        or {'a': 22, 'b': 22}
        """
        out = dict(Counter([item for t in self.edges for item in t]).most_common(1))

        return out

    def count_nodes(self) -> None:
        out = dict(Counter([item for t in self.edges for item in t]))
        count = 0
        for i in out.values():
            if i > 1:
                count += 1
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        if r1.status == 200:
            data1 = r1.read()
            data1 = json.loads(data1)['cast']
            for item in data1:
                if limit is None and exclude_ids is None:
                    ans.append(item)
                elif limit is None and exclude_ids is not None:
                    if item['id'] not in exclude_ids:
                        ans.append(item)
                elif limit is not None and exclude_ids is not None:
                    if item['id'] not in exclude_ids:
                        if item['order'] < limit:
                            ans.append(item)
                else:
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return "https://poloclub.github.io/argo-graph-lite/#c9ada5e8-9c74-4ed9-a6d1-032759db8d79"
...
if __name__ == "__main__":
    ...
    if start == 'api':
        ...
        tmdb_api_utils = TMDBAPIUtils(api_key='0cc1fdf4bd65772a6d617c9762264b2a')

        # call functions or place code here to build graph (graph building code not graded)
        # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK

        #Begin build base graph
        #Find all Laurence Fishburne movie credits with greater than 8.0 rating
        LF_Movies = TMDBAPIUtils.get_movie_credits_for_person(tmdb_api_utils, graph.nodes[0][0], 8.0)
        for LF_Movie in LF_Movies:
            Cast_Members = TMDBAPIUtils.get_movie_cast(tmdb_api_utils, str(LF_Movie['id']), limit=3)
            for Cast_Member in Cast_Members:
                graph.add_node(id=str(Cast_Member['id']), name=str(Cast_Member['name']).replace(",", ""))
                graph.add_edge(source=str(graph.nodes[0][0]), target=str(Cast_Member['id']))

        i = 0
        new_search = []
        while i < 2:
            if i == 0:
                search_nodes = graph.nodes[1:]
            else:
                search_nodes = list(set(graph.nodes[1:]) - set(search_nodes))
            for node in search_nodes:
                Movies = TMDBAPIUtils.get_movie_credits_for_person(tmdb_api_utils, node[0], 8.0)
                for Movie in Movies:
                    Cast_Members = TMDBAPIUtils.get_movie_cast(tmdb_api_utils, str(Movie['id']), limit=3, exclude_ids=[str(node[0])])
                    for Cast_Member in Cast_Members:
                        graph.add_node(id=str(Cast_Member['id']), name=str(Cast_Member['name']).replace(",", ""))
